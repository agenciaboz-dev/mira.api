...
router.post("/password-recovery", async (request: Request, response: Response) => {
    ...
    // Hotmail
    const mailOptionsHotmail = {
      from: 'seu-email@hotmail.com', 
      to: user.email, 
      subject: 'Recuperação de senha', 
      html: `
        <p>Olá ${user.name},</p>
    ...
    transporterGmail.sendMail(mailOptionsGmail, (error) => {
      if (error) {
        console.error(error);
    ...
    transporterHotmail.sendMail(mailOptionsHotmail, (error) => {
      if (error) {
        console.error(error);
        response.status(500).json({ error: "Ocorreu um erro ao enviar o e-mail de recuperação de senha." });
      } else {
